...
#include <iomanip>
#include <fstream>
#include "User.h" //include both the User and Book h files
#include "Book.h"
using namespace std; 
...
int readBooks(string filename, Book booksdata[], int numberofbooks, int capactiy)
{
    ...
    string array[2];
    int i = numberofbooks; //the counter must be initialized to the numberofbooks in order to continue adding
    //cout << "here" << endl;
...
int readRatings (string filename1, User userdata[], int numberofusers, int capacity)
{
    ...
    string array2[50]; //set array two to size 50 so it can hold each book rating
    int ratings[50];
    ...
    if (file.is_open()) //if file is open
    {
        ...
        while (getline(file, line)) //while you can still grab lines
        { 
            split(line, ',', array, 2); //split the second portion of array at each space
            username1 = array[0];
            
            capacity = 200; //set capacity to 200
            ...
            for (int j = 0; j < 50; j++)
            {
                ...
                if (stoi(holder) > 0 && stoi(holder) <= 5) //if the int value is between 0 and 5
                {
                   ratings[j] = numHolder;
                ...
                {
                   ratings[j] = 0; //ratings[j] will be 0
                }
            }
            
            userdata[i].setUsername(username1); //set the username as username1
            userdata[i].setNumRatings(numberofratings); //set ratings as numberofratings
            for(int z = 0; z < numberofratings; z++) //fill the userdata array with for loop
            {
                userdata[i].setRatingAt(z, ratings[z]); //set at index z and ratings at z
            }
            
            cout << userdata[i].getUsername() << "..." << endl; //print out each userdata
            // for(int k = 0; k < 50; k++){
...
void printAllBooks (Book booksdata[], int numBooks) //numBooks updated in case 1
{
    string title = ""; //create empty strings for title and author
    string author = "";
    
    if (numBooks == 0) //print out all of the books by their title and then author
    {
    ...
    else 
    {
        ...
        {
           title = booksdata[i].getTitle(); //assign title to its value in booksdata
           author = booksdata[i].getAuthor(); //assign author to its value in booksdata
    ...
    while (choice != "6") {
            ...
            switch (stoi(choice)) {
                ...
                case 3:
                    ...
                    {
                    printAllBooks(booksdata, numBooks); //if the file has been opened then print the books
                    }
                    else
                    {
