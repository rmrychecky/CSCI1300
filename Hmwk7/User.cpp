...
{
   if (index >= 50) //if the index is greater than 50 function should return -1 
   {
      return -1;
   }
   else 
...
{
   if (index >= 0 && index <= 200 && value >= 0 && value <= 5) //make sure the index is within capacity and ratings values
   {
      ratings[index] = value;
